...
export default function PoImportFromExcelPage() {
  ...
  const isExcelFile = (f) => {
    ...
    // Windows browsers sometimes give these:
    if (type.includes("spreadsheet") || type.includes("excel")) return true;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!files.length) {
      setError("Please select at least one Excel file (.xls or .xlsx).");
      return;
    }
    ...
    try {
      ...
      const msg =
        ...
        "Failed to import POs from Excel.";
      setError(msg);
    } finally {
      setUploading(false);
    }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1 leading-relaxed">
              Upload one or more Excel files (<span className="font-semibold">.xls / .xlsx</span>)
              and we’ll create Purchase Orders you can review and edit using the normal PO form.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="rounded-xl border-2 border-blue-200 bg-blue-50/30 p-4">
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <div className="text-sm font-semibold text-gray-900">Excel Files</div>
                  <div className="text-xs text-gray-600 mt-1">
                    Drag & drop Excel files here, or click{" "}
                    <span className="font-semibold text-blue-700">Browse</span>.
                    Each valid file creates one PO.
                  </div>
                </div>
              ...
              {files.length > 0 && (
                <div className="mt-4">
                  ...
                  <div className="mt-2 text-xs text-gray-600">
                    Tip: If you selected a non-Excel file, it will be ignored.
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="rounded-xl border-2 border-blue-200 bg-white p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="text-xs text-gray-600">
              Recommended: import 5–10 files at a time for smoother review.
            </div>
        ...
        {result && (
          <div className="mt-5 rounded-xl border-2 border-green-300 bg-green-50/40 p-4">
            ...
            {result.created?.length ? (
              <div className="mt-4">
                <div className="text-sm font-semibold text-gray-800 mb-2">Created POs</div>
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-gray-500">
                          PO created — verify vendor, dates, and line items.
                        </div>
                      </div>
            ...
            {result.errors?.length ? (
              <div className="mt-4">
                <div className="text-sm font-semibold text-red-700 mb-2">Failed Files</div>
      ...
      <div className="text-xs text-gray-500">
        If an Excel import looks incorrect, open the created PO and adjust line items before sending RFQ.
      </div>
    </div>
  );
}
